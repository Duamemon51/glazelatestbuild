...
<div class="row">
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">Edit Color Information</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.colors.update', $color) }}" method="POST" id="colorForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('color_category_id') is-invalid @enderror" 
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('color_category_id', $color->color_category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('coloring_system') is-invalid @enderror" 
                                ...
                                @foreach($coloringSystems as $key => $label)
                                    <option value="{{ $key }}" {{ old('coloring_system', $color->coloring_system) == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" 
                               ...
                               value="{{ old('closest_association', $color->closest_association) }}"
    ...
    <div class="col-md-4">
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Current Color</h6>
            </div>
            <div class="card-body text-center">
                <div class="color-preview-large mx-auto mb-3" 
                     style="background-color: {{ $color->hex_code }}; width: 120px; height: 120px;"></div>
                <h6>{{ $color->name }}</h6>
                <p class="text-muted small mb-2">{{ $color->category->name }}</p>
